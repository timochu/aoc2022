...
let result = (horizontal, vertical) ||> List.map2 (List.map2 (fun (x,y) (a,b) -> (x || a), (y * b) ))

printfn "Part 1: %i" (result |> List.sumBy(List.where (fst >> id) >> List.length))
printfn "Part 2: %i" (result |> List.concat |> List.map snd |> List.max)
